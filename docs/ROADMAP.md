...
**6. Kill the build freeze.** Mid-size builds lock the main thread for 30+ seconds (screenshots time out; the tab is unresponsive). Move layout/geometry generation into a Web Worker or chunk it across frames, with a build progress indicator in city-construction style (scaffolding, cranes — the theme invites it).
